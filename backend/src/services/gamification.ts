...
export class GamificationService {
   ...
   */
  public static async awardXp(userId: string, points: number): Promise<{ levelUp: boolean; newLevel: number; newXp: number }> {
    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user) return { levelUp: false, newLevel: 1, newXp: 0 };
   ...
   */
  private static async awardLevelBadge(userId: string, level: number) {
    if (level === 5) {
   ...
   * Award Badge to User
   */
  public static async awardBadge(userId: string, badgeName: string, description: string): Promise<void> {
    try {
   ...
   */
  public static async awardAchievement(userId: string, title: string, description: string): Promise<void> {
    try {
   ...
   */
  public static async incrementChatCount(userId: string, type: 'text' | 'voice' | 'video'): Promise<void> {
    const updateData: any = { totalChats: { increment: 1 } };
    if (type === 'voice') updateData.voiceChats = { increment: 1 };
    if (type === 'video') updateData.videoChats = { increment: 1 };
