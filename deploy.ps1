...
$cmd = "cd $dir; git fetch origin main; git reset --hard origin/main; sudo docker-compose down 2>&1; sudo docker-compose build 2>&1; sudo docker-compose rm -fs 2>&1 || true; sudo docker-compose up -d 2>&1; sudo docker image prune -f 2>&1"
